...
class Base():
    def validate_email(self, email):
        if Admin.query.filter_by(email=email.data.lower()).one_or_none() or \
        Librarian.query.filter_by(email=email.data.lower()).one_or_none():
            raise ValidationError('This email has already been registered. Please choose a different one.')
    
    def validate_username(self, username):
        if Admin.query.filter_by(username=username.data.lower()).one_or_none() or \
        Librarian.query.filter_by(username=username.data.lower()).one_or_none():
...
class ChangeUsernameForm(FlaskForm):
    ...
    def validate_new_username(self, new_username):
        if Admin.query.filter_by(username=new_username.data).one_or_none():
            raise ValidationError('This username has already been registered. Please choose a different one.')
        if Librarian.query.filter_by(username=new_username.data).one_or_none():
            raise ValidationError('This username has already been registered. Please choose a different one.')
...
class ChangeEmailForm(FlaskForm):
    ...
    def validate_new_email(self, new_email):
        if Admin.query.filter_by(email=new_email.data).one_or_none():
            raise ValidationError('This email has already been registered. Please choose a different one.')
        if Librarian.query.filter_by(email=new_email.data).one_or_none():
            raise ValidationError('This email has already been registered. Please choose a different one.')